...
import static org.sonatype.nexus.repository.r.internal.RPackagesUtils.parseMetadata
  ...
  def 'Properly parse metadata'() {
    when:
      List<Map<String, String>> metadata =
          getClass().getResourceAsStream('PACKAGES').withCloseable { input -> parseMetadata(input) }
    then:
      metadata == [
          [
              Package         : 'A3',
