    ...
    extends RITSupport
{
  ...
  public void setup() throws Exception {
    ...
    server = Server.withPort(0)
        ...
        .serve("/" + AGRICOLAE_FULL).withBehaviours(file(testData.resolveFile(AGRICOLAE_FILE_NAME)))
  ...
  public void fetchTgzPackageFile() throws Exception {
    assertSuccessResponseMatches(client.fetch(AGRICOLAE_FULL), AGRICOLAE_FILE_NAME);
    ...
    Assert.assertThat(asset.format(), is(equalTo(R)));
  }
  ...
  public void checkComponentCreated() throws Exception {
    final Component nullComponent = findComponent(repository, AGRICOLAE);
    Assert.assertThat(nullComponent, is(nullValue()));
    client.fetch(AGRICOLAE_FULL);

    final Component component = findComponent(repository, AGRICOLAE);
    Assert.assertThat(component.name(), is(equalTo(AGRICOLAE)));
    Assert.assertThat(component.format(), is(equalTo(R)));
